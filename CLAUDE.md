...
**Все ответы и объяснения — на русском языке.**

- Все мои сообщения, объяснения, рассуждения, описания плана — по-русски
- Сообщения git-коммитов — по-русски (например: «feat: добавлен автобэкап БД», «fix: исправлен расчёт стрика»)
- Комментарии и docstring в коде, которые я добавляю/правлю — по-русски
- Сообщения для пользователей внутри приложения (бот, UI) — по-русски
- Технические термины можно оставлять на английском (API, deploy, commit, push, endpoint и т. п.) — это нормально и привычно
- Имена переменных, функций, классов и файлов — на английском, как принято в коде
